...
import { construirePayloadPack } from "@/lib/produits/pack-rules";
import {
  creerProduit,
  listerLocations,
...
export async function POST(request: Request) {
  ...
  if (!infos.nom || !infos.prix)
    return NextResponse.json({ error: "champs requis : nom, prix" }, { status: 400 });

  try {
    const payload = construirePayloadPack(infos);
    const creation = await creerProduit(payload);
    if (!creation.ok)
      return NextResponse.json(
        { etape: "création pack", erreur: creation.data },
