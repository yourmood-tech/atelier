...
const ACTION_REFS: Record<string, { folder: string; count: number }> = {
  "fond-blanc": { folder: "fond-blanc", count: 5 },
};
...
const PROMPTS: Record<string, string> = {
  "fond-blanc": "MOOD CATALOG PACKSHOT — Re-photograph the ring on a pure white catalog background, in the exact signature Mood catalog angle.\n\n🎯🎯🎯 MULTIPLE IMAGES PROVIDED — IDENTIFICATION :\n📐 IMAGE 1 (first attached) = ANGLE REFERENCE → COPY this camera angle / ring pose / orientation / framing EXACTLY. The ring in this reference image is lying flat horizontally, viewed from a slight 3/4 horizontal angle with a tiny downward tilt, the inner polished hole visible on the right side as an oval, the decorated band-surface visible on the top. IGNORE this reference's ring design/colors.\n💎 IMAGE 2 (second attached) = THE RING TO PHOTOGRAPH → preserve its identity exactly (shape, colors, materials, gemstones, engravings, pattern). IGNORE its current pose/angle.\n\nYOUR TASK : Output IMAGE 2's ring photographed at IMAGE 1's exact angle, on a pure white catalog background.\n\n═══════════════════════════════════════════════\n📷 SHOT SPECIFICATIONS\n═══════════════════════════════════════════════\n\n✂️ PIXEL-PRECISE CUTOUT on PURE WHITE :\n- 100% pure white background (#FFFFFF) — NOT gray, NOT off-white, NOT cream. Exactly RGB 255,255,255.\n- Edges cut at the EXACT pixel boundary of the ring. No fuzzy edge, no halo, no colored fringe, no semi-transparent pixels.\n- Preserve every intricate detail: engravings, gemstones, prongs, small openings, the inner ring hole (background shows through it as pure white).\n- NO cast shadow on the surface. NO floor shadow. The ring's own 3D self-shadow (modeling the volume) can stay.\n\n📐 ANGLE & FRAMING (match IMAGE 1 reference) :\n- Camera at near eye-level with a very slight downward tilt (~5-10°).\n- Ring lies flat horizontally, slightly tilted to show the inner polished hole as a wide horizontal oval at one end.\n- The decorated outer band-surface is visible on the top of the ring, the polished side rails frame it.\n- Format: SQUARE 1:1.\n- The ring fills 85-95% of the frame width, centered.\n\n✨ MOOD QUALITY STANDARDS :\n- ✂️ REMOVE all dust, lint, fingerprints, scratches, surface marks, fluff, water spots, residues, oily traces.\n- ✨ Polished metal surfaces are MIRROR-CLEAN with crisp clean reflections.\n- 💎 Gemstones sparkle clean and brilliant.\n- 🎯 Addon-base structure: addon fills the entire central groove between the two polished rails, no overflow, no gaps, flush at the same surface height (no relief/step), uniform width along the entire visible length.\n- 💡 Lighting: even soft studio illumination from above and slightly to one side, creating subtle gradient reflections on the polished metal — NOT harsh hotspots, NOT flat ringlight, just clean even product light that reveals every detail.\n- 📏 Ring proportions identical to IMAGE 2 source — same band thickness, same width balance between rails and addon.\n\n⛔ ABSOLUTE BANS :\n- NO gray, off-white or cream background — only #FFFFFF pure white.\n- NO cast shadow on the surface.\n- NO redesigning the ring or its decoration.\n- NO changing the angle from IMAGE 1's reference.",
...
async function appelGemini(imageDataUrl: string, action: string, note?: string | null): Promise<{ image?: string; error?: string }> {
  ...
  // Si l'action a une référence d'angle hardcodée, on l'ajoute EN PREMIER (Gemini priorise la 1ère image)
  const refPart = loadActionReference(action);
  const parts = refPart
    ? [refPart, { inlineData: { mimeType, data } }, { text: prompt }]
