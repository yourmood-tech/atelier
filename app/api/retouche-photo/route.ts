...
const PROMPTS: Record<string, string> = {
  "fond-blanc": "ISOLATE ONLY THE RING from this image. Remove COMPLETELY everything else : the background, the surface on which the ring sits, any shadow cast on the surface, any prop, any hand or finger, any fabric, any support stand, any second object, any reflection of surroundings on the surface. Output the ring ALONE, perfectly cut out, centered, on a pure uniform pristine white background (#ffffff). The ring itself can keep its own intrinsic shadow and depth (the shadow that is part of its 3D volume), but the surface it was resting on must be entirely gone — replaced by pure white. Clean cut-out edges, no halo, no colored fringe, no semi-transparent pixel from the previous background. The ring stays unchanged in shape, color, material and finish — only it is preserved, nothing else.",
