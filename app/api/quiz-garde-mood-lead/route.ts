...
// Inscription publique d'une cliente (e-mail donné à la fin du quizz garde-mood)
...
export async function POST(req: Request) {
  try {
    ...
    const create = await kfetch("https://a.klaviyo.com/api/profiles/", {
      ...
      body: JSON.stringify({
        data: {
          ...
          attributes: {
            email,
            properties: pack ? { QuizGardeMood: pack } : undefined,
          },
        },
      }),
    });
    ...
    if (!id) {
      console.error("[quiz-garde-mood-lead] pas d'id profil");
    ...
    kfetch("https://a.klaviyo.com/api/profile-subscription-bulk-create-jobs/", {
      ...
      body: JSON.stringify({
        data: {
          ...
          attributes: {
            custom_source: "Quizz garde-mood" + (pack ? ` — ${pack}` : ""),
            profiles: {
              data: [
                {
                  type: "profile",
    ...
    console.error("[quiz-garde-mood-lead] exception:", e instanceof Error ? e.message : e);
